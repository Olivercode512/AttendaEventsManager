# main.py – Versión que SOLUCIONA el RuntimeError global scope
...
def login_page():
    ...
    with ui.card().classes('w-96 mx-auto mt-10 p-8 shadow-2xl rounded-xl'):
        ...
        def try_login():
            print(f"[DEBUG] Intento login → email: {email.value}")
...
def dashboard_page():
    ...
    ui.notify("Sesión iniciada correctamente", type='positive')
...
ui.run(
    ...
    port=8080,
    reload=True,
    storage_secret="cambia_esto_por_tu_clave_real_larga_y_unica_2026"  # ← CAMBIA ESTO
)